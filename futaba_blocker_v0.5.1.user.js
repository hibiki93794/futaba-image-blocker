...
// @version      0.5.1 // 正規表現エラー修正 & 全体チェック
...
(function() {
    ...
    const TEXT_NG_RULES = {
        MIN_TEXT_LENGTH_FOR_PATTERNS: 10,
        MAX_DIGITS_SEQUENCE: 50,
        MAX_ASCII_SYMBOLS_SEQUENCE: 30,
        MAX_SINGLE_CHAR_SEQUENCE: 30,
        MIN_LENGTH_FOR_EMOJI_RULE: 10,
        EMOJI_PERCENTAGE_THRESHOLD: 0.8,
        MAX_ALPHABET_ONLY_SEQUENCE: 70
    };
    ...
    async function calculateDHash(imageUrl) { /* v0.5.0 と同じ */
    ...
    function hammingDistance(hash1, hash2) { /* v0.5.0 と同じ */ if (!h1 || !h2 || h1.length !== h2.length) return Infinity; let d = 0; for (let i = 0; i < h1.length; i++) if (h1[i] !== h2[i]) d++; return d; }

    // --- テキストNGパターン判定ヘルパー関数群 ---
    function isTooLongDigits(text) { return new RegExp(`\\d{${TEXT_NG_RULES.MAX_DIGITS_SEQUENCE},}`).test(text); } // `}` を一つ削除
    function hasTooManyAsciiSymbols(text) { return new RegExp(`[!"#$%&'()*+,-./:;<=>?@[\\]^_\`{|}~]{${TEXT_NG_RULES.MAX_ASCII_SYMBOLS_SEQUENCE},}`, 'u').test(text); } // `}` を一つ削除、'u'フラグ確認
    function isSingleCharTooLong(text) { return new RegExp(`(.)\\1{${TEXT_NG_RULES.MAX_SINGLE_CHAR_SEQUENCE - 1},}`, 'u').test(text); } // `}` を一つ削除
    ...
    function isLongAlphabetOnly(text) { return new RegExp(`^[a-zA-Z]{${TEXT_NG_RULES.MAX_ALPHABET_ONLY_SEQUENCE},}$`).test(text); } // `}` を一つ削除

    // --- レス全体を処理する関数 ---
    async function processPostRow(postRowElement) {
        if (!postRowElement || !postRowElement.matches || !postRowElement.matches('tr') ||
            postRowElement.classList.contains('futaba-content-processed-final') ||
            !postRowElement.closest('body')) {
            return;
        }
        if (postRowElement.style.display === 'none') { // 既に非表示なら何もしない
            postRowElement.classList.add('futaba-content-processed-final');
            return;
        }

        postRowElement.classList.add('futaba-content-processing');
        ...
        if (ngReason) {
            ...
            placeholderRow.classList.add('futaba-content-processed-final');
        ...
        postRowElement.classList.remove('futaba-content-processing');
    }

    // --- メイン処理 (スキャンと監視) ---
    function scanAndProcessPosts(parentNode) {
        if (!parentNode || !parentNode.querySelectorAll) return; // parentNodeの存在とメソッド確認
        parentNode.querySelectorAll('tr:not(.futaba-content-processed-final):not(.futaba-content-processing)').forEach(tr => {
    ...
    if (document.readyState === 'loading') {
        ...
        if (document.body) { // document.body が利用可能か確認
            scanAndProcessPosts(document.body);
        } else { // bodyがまだない場合はDOMContentLoadedを待つ
            document.addEventListener('DOMContentLoaded', () => scanAndProcessPosts(document.body));
        }
    }
    ...
    document.addEventListener('mouseover', function(event) { if (event.target && event.target.tagName === 'IMG') lastHoveredImageElement = event.target; }, true); // event.targetの存在確認
            ...
            try {
                ...
                if (hash) {
                    if (!blockedImageHashes.includes(hash)) {
                        ...
                        if (postRowToProcess) {
                            postRowToProcess.classList.remove('futaba-content-processing', 'futaba-content-processed-final');
    ...
    GM_registerMenuCommand("■ 現在のブロックハッシュ数を確認", () => { alert(`現在 ${blockedImageHashes.length} 件のハッシュがブロックリストに登録されています。`); });

    console.log('%c[不快コンテンツブロッカー (ふたばちゃんねる用 v0.5.1)]%c が動作を開始。正規表現エラー修正 & 全体チェック。', "color:orange;font-weight:bold;", "color:default;");
