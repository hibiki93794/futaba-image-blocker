...
(function() {
    ...
    function hasTooManySpamSymbols(text) { if (new RegExp(`[!"#$%&'()*+,-./:;<=>?@[\\]^_\`{|}~]{${TEXT_NG_RULES.MAX_SYMBOLS_SEQUENCE},}}`).test(text)) return true; if (new RegExp(`[\u2000-\u2BFF\u2E00-\u2E7F]{${TEXT_NG_RULES.MAX_UNICODE_SYMBOLS_SEQUENCE},}}`, 'u').test(text)) return true; return false; }
    ...
    async function processPostElement(postElement) {
        if (!postElement || postElement.classList.contains('futaba-content-processed') || !postElement.closest('body')) {
            return;
        }
        const postRow = postElement.tagName === 'TR' ? postElement : postElement.closest('tr');
        if (!postRow || postRow.classList.contains('futaba-content-processed')) {
             if(postElement.tagName === 'IMG') postElement.classList.add('futaba-image-scan-attempted');
            return;
        }
        ...
        if (ngReason) {
            postRow.style.display = 'none'; // レスを非表示
            let resNoText = "";
    ...
    console.log('%c[不快コンテンツブロッカー (ふたばちゃんねる用 v0.7)]%c が動作を開始。非表示時メッセージなし。インポート/エクスポート機能付き。', "color:orange;font-weight:bold;", "color:default;");
})();