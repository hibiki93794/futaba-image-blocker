// ==UserScript==
// @name         不快画像ブロッカー (ふたばちゃんねる用) // ★★★ @name を確実に設定 ★★★
// @namespace    http://tampermonkey.net/
// @version      0.4.4-namefix // @name修正確認用バージョン
...
(function() {
                ...
                onload: function(response) {
                    ...
                    img.onload = () => {
                        try {
                            ...
                            // processingImages.delete(imageUrl); // finallyブロックに移動済み (v0.4.4時点ではここだったかも)
                            resolve(hash);
                        } catch (e) {
                            // processingImages.delete(imageUrl); // finallyブロックに移動済み
                            reject(new Error(`dHash計算中のエラー: ${e.message} for ${imageUrl}`));
                        } finally {
                             if(img.src.startsWith('blob:')) URL.revokeObjectURL(img.src); // blob URLのみ解放
                             processingImages.delete(imageUrl); // 正常終了でもエラーでも削除
                        }
                    };
                    img.onerror = () => {
    ...
    function hammingDistance(hash1, hash2) {
        if (!hash1 || !h2 || hash1.length !== hash2.length) {
            return Infinity;
        }
        let distance = 0;
    ...
    async function processImageElement(imgElement) {
        ...
        try {
            ...
            for (const blockedHash of blockedImageHashes) {
                ...
                if (distance <= SIMILARITY_THRESHOLD) {
                    ...
                    if (postRow && !postRow.classList.contains(postProcessedClass)) {
                        ...
                        console.log(`%c[不快画像ブロッカー v0.4.4-namefix]%c ${resNoText}の内容を非表示 (不快画像を検出、距離: ${distance})。画像URL: ${imageUrl} (ハッシュ元: ${effectiveImageUrl.substring(0,60)}...)`, "color:orange;font-weight:bold;", "color:default;");
                        ...
                        console.log(`%c[不快画像ブロッカー v0.4.4-namefix]%c 類似画像 (距離: ${distance}) を検出。画像のみ非表示: ${imageUrl} (trが見つからず)`, "color:orange;font-weight:bold;", "color:default;");
            ...
            console.warn(`[不快画像ブロッカー v0.4.4-namefix] 画像処理エラー: ${error.message}`, `(対象URL: ${effectiveImageUrl})`);
            imgElement.classList.remove('futaba-image-processed');
        }
    }
            ...
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === Node.ELEMENT_NODE) {
                        ...
                        else if (node.tagName === 'IMG' && node.parentNode) scanImagesInNode(node.parentNode); // 親をスキャン
                    }
                });
            }
        }
    ...
    document.addEventListener('mouseover', function(event) {
        if (event.target && event.target.tagName === 'IMG') { // event.targetの存在確認
        ...
        if (lastHoveredImageElement && lastHoveredImageElement.src && lastHoveredImageElement.closest('body')) {
            ...
            if (imgToBlock.parentNode && imgToBlock.parentNode.tagName === 'A' && imgToBlock.parentNode.href) {
                ...
                if (/\.(jpe?g|png|gif|webp)$/i.test(linkUrl) && !linkUrl.includes('futaba.htm') && !linkUrl.includes('/res/')) {
                    ...
                    console.log(`%c[不快画像ブロッカー v0.4.4-namefix]%c 登録時: サムネイル ${originalSrc.substring(0,60)}... の代わりにリンク先 ${imageToHashUrl.substring(0,60)}... を使用`, "color:orange;font-weight:bold;", "color:default;");
                }
            }
            ...
            try {
                ...
                if (hash) {
                    if (!blockedImageHashes.includes(hash)) {
                        ...
                        if (postRowToProcess) { // 確実に postRowToProcess が存在する場合のみ処理
                            if (postRowToProcess.classList.contains('futaba-post-processed-by-blocker')) {
                            ...
                            await processImageElement(imgToBlock); // imgElement を渡す
                        }
                    } else {
                ...
                console.error("[不快画像ブロッカー v0.4.4-namefix] ハッシュ登録エラー:", error);
            }
        } else {
    ...
    GM_registerMenuCommand("■ 現在のブロックハッシュ数を確認", () => {
        alert(`現在 ${blockedImageHashes.length} 件のハッシュがブロックリストに登録されています。`);
    });

    console.log('%c[不快画像ブロッカー (ふたばちゃんねる用 v0.4.4-namefix)]%c が動作を開始しました。', "color:orange;font-weight:bold;", "color:default;");
