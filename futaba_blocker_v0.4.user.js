...
// @license      MIT // ふたば☆ちゃんねるのMAYのオルガスレで開発配布したものであり団員ならコードの利用、変更、再配布、商用利用許可しますからよ…
// ==/UserScript==
            ...
            for (const blockedHash of blockedHashes) {
                ...
                if (distance <= SIMILARITY_THRESHOLD) {
                    ...
                    if (postRow && !postRow.classList.contains(postProcessedClass)) {
                        ...
                        placeholderText.textContent = `[${resNoText}の内容に不快な可能性のある画像が含まれていたため非表示にしました (類似度:${distance})]`;
