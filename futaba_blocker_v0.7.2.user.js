...
// @license      MIT ふたば☆ちゃんねるのMAYで開発配布したものでありとしあきならコードの利用、変更、再配布、商用利用許可します
// ==/UserScript==
...
(function() {
    ...
    async function processPostElement(postElement) {
        ...
        if (blockquote && blockquote.ownerDocument === document) { // blockquoteも現在のドキュメントに属しているか
        ...
        if (!ngReason) {
            ...
            if (imgElement && imgElement.ownerDocument === document && imgElement.src) { // imgElementも現在のドキュメントに属しているか
    ...
    function scanAndProcessPostsInNode(parentNode) {
        if (parentNode.ownerDocument !== document) return; // 親ノードが現在のドキュメントに属しているか
        parentNode.querySelectorAll('tr:not(.futaba-content-processed)').forEach(tr => {
            if (tr.ownerDocument !== document) return; // trが現在のドキュメントに属しているか
        ...
        for (const mutation of mutationsList) {
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === Node.ELEMENT_NODE && node.ownerDocument === document) { // 追加されたノードが現在のドキュメントに属しているか
                        ...
                        } else if (node.querySelector && node.querySelector('tr:not(.futaba-content-processed)')) {
                            scanAndProcessPostsInNode(node);
                        }
                    }
                });
    ...
    document.addEventListener('mouseover', (event) => {
        if (event.target && event.target.ownerDocument === document && event.target.tagName === 'IMG') { // 対象が現在のドキュメントに属しているか
                ...
                if (hash) {
                    if (!blockedImageHashes.includes(hash)) {
                        ...
                        if (postRowToProcess?.classList.contains('futaba-content-processed')) {
                             ...
                             if (nextSibling?.classList.contains('futaba-content-processed')) nextSibling.remove();
                        }
                         if(imgToBlock.classList.contains('futaba-image-scan-attempted')) imgToBlock.classList.remove('futaba-image-scan-attempted');
                        if (postRowToProcess && postRowToProcess.ownerDocument === document) await processPostElement(postRowToProcess); // レス全体を再処理
    ...
    GM_registerMenuCommand("■ 登録済み不快画像ハッシュを全てクリア", () => { /* 変更なし */ });
    GM_registerMenuCommand("■ 現在のブロックハッシュ数を確認", () => { /* 変更なし */ });
    function exportBlockedHashes() { /* 変更なし */ }
    function importBlockedHashes() { /* 変更なし */ }
    GM_registerMenuCommand("💾 画像ハッシュリストをエクスポート", exportBlockedHashes);
    GM_registerMenuCommand("📂 画像ハッシュリストをインポート", importBlockedHashes);

    console.log('%c[不快コンテンツブロッカー (ふたばちゃんねる用 v0.7.2)]%c が動作を開始。クロスオリジン対策試行。', "color:orange;font-weight:bold;", "color:default;");
})();