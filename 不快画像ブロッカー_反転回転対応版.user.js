...
(function() {
    ...
    async function calculateDHash(imageUrl) {
        if (!imageUrl || imageUrl.startsWith('data:') || processingImages.has(imageUrl)) return null;
    ...
    async function processImageElement(imgElement) {
        ...
        try {
            const currentHashes = await calculateDHash(effectiveImageUrl);
            if (!currentHash) {
            ...
            for (const blockedHash of blockedHashes) {
                ...
                const distance = hammingDistance(h, blockedHash);
                if (distance <= SIMILARITY_THRESHOLD) { break; }
